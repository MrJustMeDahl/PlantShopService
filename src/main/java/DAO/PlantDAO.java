...
public class PlantDAO implements IDAO<PlantDTO, Integer, Plant>, IDAOType<Plant, String> {
    ...
    public Plant delete(int id) throws APIException {
        ...
        try (EntityManager em = emf.createEntityManager()) {
            found = em.find(Plant.class, id);
            if (found != null) {
                Query query = em.createNativeQuery("SELECT reseller_id FROM public.reseller_plant WHERE plants_id = ?");
                query.setParameter(1, found.getId());
                List<Integer> resellerIDs = query.getResultList();
                if (!resellerIDs.isEmpty()) {
                    Set<Reseller> resellers = new HashSet<>();
                    for (Integer i: resellerIDs) {
                        Reseller reseller = em.find(Reseller.class, i);
                        reseller.removePlant(found);
